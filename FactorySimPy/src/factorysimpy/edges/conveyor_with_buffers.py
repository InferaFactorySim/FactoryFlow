...
# @title beltstore_inter_reserve_get modified--- external buffers- user can yield reserve reqs and instantaneously get or put items.
...
class ConveyorBelt(Edge):
  ...
  def __init__(self,env,id,belt_capacity,delay_per_slot,accumulating=0):
    ...
    self.item_put_event = self.env.event()
    self.inbuiltstore = ReservablePriorityReqFilterStore(env, capacity=1)
    self.outstore = ReservablePriorityReqFilterStore(env, capacity=1)
    ...
    self.env.process(self.reserve_get())
    self.env.process(self.reserve_put())
  ...
  def _handle_put_request(self):
      ...
      yield put_event
      self.belt.reserve_put_cancel(put_event)
  ...
  def _handle_get_request(self):
      if self.get_request_queue:
        get_event = self.belt.reserve_get()
        yield get_event
        self.belt.reserve_get_cancel(get_event)
        #print(f"T={env.now:.2f}: conveyor yielded get request {get_event}")
  ...
  def _handle_get_main(self):
        ...
        while True:
          ...
          g = self.belt.reserve_get()
          yield g
          item = self.belt.get(g)
          self.outstore.put(put_event, item)




  def handle_put_main(self):
        """Queue a put request or add item to the belt if possible."""
        while True:
          #dont worry about user canceling, because it happend outside 
          # reserve_get yields only if an item is available.
          g=self.inbuiltstore.reserve_get()
          yield g #yielding from inbuiltstore
          ...
          if len(self.put_request_queue)==1 and len(self.belt.items)==0 and self.can_put():
            ...
            if len(self.put_request_queue)==1 and len(self.belt.items)!=0 and self.can_put():
              print(f"T={self.env.now:.2f}: Conveyor '{self.id}':reserve_put: put request queue=={len(self.put_request_queue) }and check nect event ")
              if not self.item_put_after_a_break_event.triggered: # Check if the event has already been triggered
                print(f"T={self.env.now:.2f}: Conveyor '{self.id}':reserve_put: put request queue=={len(self.put_request_queue) }and self.item_put_after_a_break_event and check for previous event ")
          ...
          item = self.inbuiltstore.get(g) #getting item from inbuitlstore
          put=self.belt.reserve_put()
          yield put
          self.belt.put(put,item) #putting item
          self.item_put_inslot = True
